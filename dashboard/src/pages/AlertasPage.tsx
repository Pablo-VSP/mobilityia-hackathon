  ...
  return (
    <div className="h-full overflow-y-auto p-6 bg-slate-50">
      <div className="max-w-4xl mx-auto">
        ...
        {loading && <p className="text-slate-400">Cargando alertas...</p>}
        ...
        <div className="space-y-4">
          {groups.map(group => {
            ...
            return (
              <div key={group.autobus} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                {/* Group header */}
                <div
                  className={`p-5 ${hasMultiple ? 'cursor-pointer hover:bg-slate-50' : ''}`}
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <p className="text-slate-500 text-sm">{group.viaje_ruta}</p>
                    </div>
                  ...
                  <p className="text-slate-700 mb-3">{latest.diagnostico}</p>
                ...
                {isExpanded && hasMultiple && (
                  <div className="border-t border-slate-200">
                    ...
                    {sorted.map((a, idx) => (
                      <div
                        key={a.alerta_id}
                        className={`px-5 py-3 ${idx > 0 ? 'border-t border-slate-100' : ''} ${idx === 0 ? 'bg-slate-50/50' : ''}`}
                      >
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            <StatusBadge status={a.nivel_riesgo} />
                            <span className="text-xs text-slate-400">{a.numero_referencia}</span>
                          </div>
                          <span className="text-xs text-slate-400">
                            {new Date(a.timestamp).toLocaleString('es-MX')}
                          </span>
                        </div>
                        <p className="text-slate-600 text-sm mb-1">{a.diagnostico}</p>
                        <div className="flex items-center gap-2 text-xs text-slate-400">
                          <Clock className="w-3 h-3" />
                          <span>{a.urgencia.replace(/_/g, ' ')}</span>
                          {a.componentes.length > 0 && (
                            <span className="text-slate-300">· {a.componentes.join(', ').replace(/_/g, ' ')}</span>
                          )}
                        </div>
                      </div>
                    ))}
                ...
                <div className="px-5 py-3 border-t border-slate-200 bg-slate-50">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2 bg-red-50 hover:bg-red-600 text-red-600 hover:text-white text-sm font-medium rounded-lg transition-colors"
                  >
